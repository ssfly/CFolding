...
{
	_InWhichLoop = -1;
	_V1H = MyMesh::VertexHandle(-1);
	_V2H = MyMesh::VertexHandle(-1);
}
...
Point LineSegment::GetP(int i)
{
	if (i == 1)
		...
		assert(i >= 3, "No more than 2 points in one linesegment!");
}
...
MyMesh::VertexHandle  LineSegment::GetV1Handle()
{
	return _V1H;
}

void LineSegment::SetV1Handle(MyMesh::VertexHandle h)
{
	_V1H = h;
}

MyMesh::VertexHandle LineSegment::GetV2Handle()
{
	return _V2H;
}

void LineSegment::SetV2Handle(MyMesh::VertexHandle h)
{
	_V2H = h;
}

void LineSegment::SetVHandle(int i, MyMesh::VertexHandle  h)
{
	if (i == 1)
		_V1H = h;
	else if (i == 2)
		...
		assert(i >= 3,"No more than 2 vertices in one linesegment!");
}
